...
public:
	...
	void init_KModel(string file_kmer_data_base) {
		...
		while (kmer_data_base.ReadNextKmer(kmer_object, counter)) {
			...
			//cout << kmer << endl;
			occ32 = counter > C_MAX - 1 ? C_MAX - 1 : counter;
	...
	//	int bin = find_in_bit_array(kmer);
	//	if (bin > -1) {
	//		//bin_to_mean/////////////////
	//		return occu_bin->bin_to_mean(bin);
	//	}
	//	return find_in_map(kmer);
	//}
	...
	int kmer_to_bin(string kmer, int &len_v_bin, int gt_bin) {
		vector<int> v_bin = find_bitarray(kmer);
	...
	void save_model(string save_dir) {
		...
		for (int i = 0; i < n_bits; i++) {
			...
			fwrite(bits_data[i].bit_array_1, sizeof(uint8_t), byte_array_size, fp_bit1);
			fclose(fp_bit1);
			//3.3 bit_array_2
			FILE *fp_bit2 = fopen((save_dir + "/bit2_" + std::to_string(i) + ".bin").c_str(), "wb");
			fwrite(bits_data[i].bit_array_2, sizeof(uint8_t), byte_array_size, fp_bit2);
			fclose(fp_bit2);
		}
	...
	void load_model(string save_dir) {
		...
		set_bit_array_size(kmer_count);
		fin.close();
		//cout << "n_hash " << n_hash << " kmer_count " << kmer_count << endl;
		...
		for (int i = 0; i < n_bits; i++) {
			...
			fclose(fp_hash);
			//3.2 bit_array_1
			bits_data[i].bit_array_1 = new uint8_t[byte_array_size];
			FILE *fp_bit1 = fopen((save_dir + "/bit1_" + std::to_string(i) + ".bin").c_str(), "rb");
			fread(bits_data[i].bit_array_1, sizeof(uint8_t), byte_array_size, fp_bit1);
			fclose(fp_bit1);
			//3.3 bit_array_2
			bits_data[i].bit_array_2 = new uint8_t[byte_array_size];
			FILE *fp_bit2 = fopen((save_dir + "/bit2_" + std::to_string(i) + ".bin").c_str(), "rb");
			fread(bits_data[i].bit_array_2, sizeof(uint8_t), byte_array_size, fp_bit2);
			fclose(fp_bit2);
		}
