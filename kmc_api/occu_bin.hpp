...
const int C_MAX = 1001+1; //the max occurrence,exclude self
...
private:
	...
	uint64_t kmer_count;
	OccuBinMeta occ_bin_meta[C_MAX];
	int occ_count[C_MAX] = {0};
	...
	void init_occ_bin() {
		...
		cout << "width_start_bin:" << width_start_bin << endl;
		...
		printf("%d %d %d %d %d\n", bin_capacity, C_MAX, width_start, max_bin, width_start_bin);
